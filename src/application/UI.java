...
public class UI {
	...
	public static PosicaoXadrez lerPosicaoXadrez(Scanner sc) {
		try {
			...
			throw new InputMismatchException("Erro ao ler posição de Xadrez. Valores validos de a1 a h8");
		}
	...
	public static void printPartida(PartidaXadrez partida, List<PecaXadrez> capturada) {
		...
		System.out.println();
		System.out.println("Vez de : " + partida.getVez());
	...
	public static void printTabuleiro(PecaXadrez[][] pecas, boolean[][] movPossiveis) {
		for(int i = 0; i < pecas.length; i++) {
			System.out.print(8 - i + " ");
			for(int j = 0; j < pecas.length; j++) {
				printPeca(pecas[i][j], movPossiveis[i][j]);
			}
			System.out.println();
		}
		System.out.println("  a b c d e f g h");
	...
	private static void printPecasCapturadas(List<PecaXadrez> capturada) {
		List<PecaXadrez> white = capturada.stream().filter(x -> x.getCor() == Cor.BRANCO).collect(Collectors.toList());
		List<PecaXadrez> black = capturada.stream().filter(x -> x.getCor() == Cor.PRETO).collect(Collectors.toList());
